...
export default function Tablero() {
    ...
    useEffect(() => {
        setMap((prevState) => ({
            ...prevState,
            areas: 
                tableroMap.areas.filter(area => cases.includes(area.id) || area.shape === "player").concat(...prevState.areas.slice(-players.length))
        }));
    },[cases])
    ...
    // LOOP
    // 1.Check if it the turn of the user and set question
    useEffect(() => {
        socketService.turn( (data) => {
            ...
            //get and set players positio from data.stats
            Object.keys(data.stats).forEach(player => {
                players.forEach((p,index) => {
                    if (p.nickname === player) {
                        players[index].position = data.stats[player].position;
                        players[index].totalAnswers = data.stats[player].totalAnswers;
                        players[index].correctAnswers = data.stats[player].correctAnswers;
                        players[index].tokens = data.stats[player].tokens;
                    }
                })
            })
        })
   ...
   const handleMovement = (area) => {
       ...
       socketService.makeMove({rid, pos}, (data) => {
           if (data.ok === false) {
               ...
               data.rollAgain ? setDice(true) : setDice(false);
           }
           setCases([])
       })
   }
    ...
    return (
        ...
        >
            ...
            <Grid item container xs={12} lg={10}
                ...
                sx={{backgroundColor:'accent.main', position:'relative', minHeight:'calc(50vh - 64px)'}}
            >           
                {map !== undefined && !loading && 
                    <ImageMapper
                        ...
                        src={tableroURL}
                        map={map}
                        onClick={(area) => { handleMovement(area) }}
                    />
                }
                <Dialog  
                    open={!!dice} 
                ...
                >
                    <Box sx={{p:3, backgroundColor: 'transparent'}}>
                        <Dice size="75" cheatValue={diceData.roll} rollingTime={1500} onRoll={() => handleReachableCases()}/>
                    </Box>
                </Dialog>
            ...
            {/* Side */}
            <Grid item container xs lg={2} direction="column" sx={{backgroundColor:'light.main'}}>
                {/* Categories */}
                <Grid item container spacing={1} sx={{alignContent:'center', justifyContent:'space-around',pt:2}}>
                    <Grid item xs={4} lg={5}>
                    ...
                    <Grid item xs={4} lg={5}>
                        <Paper sx={{display:'flex',justifyContent:'center',alignItems:'center', backgroundColor:'pink'}}>
                            <Avatar
                ...
                {/* Players */}
                <Grid item container xs direction='column' sx={{alignContent:'center', justifyContent:'space-around',pt:2}}>
                    {players.map((player, index) => {
                        return (
                            <Grid container item alignItems='center' sx={{px:2}}>
                                <Avatar
                                    src={process.env.PUBLIC_URL + '/images/cosmetics/cosmetic_'+player.avatar+'.jpg'}
                                    alt="Avatar"
                                    sx={{width: '50px', height: '50px', mx:1, border: '2px solid '+ colors[index]}}
                                />
                                <Typography variant="body">{player.nickname}</Typography>            
                            </Grid>
                        )
                    })}
                </Grid>
            ...
            >
                <QuestionMulti 
                    ...
                    onCloseDialog={() => { setQuestion(false) }}
                />
            </Dialog>
        </Grid>
    );
